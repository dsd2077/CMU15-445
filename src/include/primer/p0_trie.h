...
class TrieNode {
 public:
   ...
   */
  explicit TrieNode(char key_char = '\0') : key_char_(key_char) {}
  ...
  std::unique_ptr<TrieNode> *InsertChildNode(char key_char, std::unique_ptr<TrieNode> &&child) {
    if (!child || children_.find(key_char) != children_.end() || key_char != child->GetKeyChar()) {
      return nullptr;
    }
...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    for (; i < key.size() - 1; i++) {
      ...
      if ((*cur)->HasChild(c)) {
        ...
        // auto temp  = std::make_unique<TrieNode>(c);
        std::unique_ptr<TrieNode> temp(new TrieNode(c));
        (*cur)->InsertChildNode(c, std::move(temp));
        cur = (*cur)->GetChildNode(c);
      }
    }
    ...
    if (temp == nullptr) {
      ...
      (*cur)->InsertChildNode(last_char, std::move(node));
  ...
  bool Remove(const std::string &key) {
    ...
    path.pop_back();  // 将最后一个节点删除

    // 终端节点还有子节点，只需将is_end设置为false
    if ((*cur)->HasChildren()) {
      (*cur)->SetEndNode(false);
    } else {
      // 递归删除终端节点
      while (!path.empty()) {
        std::unique_ptr<TrieNode> *father = path.back();
        path.pop_back();
        (*father)->RemoveChildNode((*cur)->GetKeyChar());
        // 还有其他孩子节点，或者本身为终端节点，删除结束
        if ((*father)->HasChildren() || (*father)->IsEndNode()) {
          latch_.WUnlock();
          return true;
        }
        cur = father;
  ...
  T GetValue(const std::string &key, bool *success) {
    *success = false;
    ...
    auto temp = dynamic_cast<TrieNodeWithValue<T> *>(cur->get());
    if (temp == nullptr) {
      latch_.RUnlock();
      return {};
    }
