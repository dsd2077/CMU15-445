...
auto NestIndexJoinExecutor::ConstructJoinTuple(Tuple left_tuple, const Schema &left_schema, Tuple right_tuple, const Schema &right_schema) -> Tuple {
...
auto NestIndexJoinExecutor::ConstructNullJoinTuple(Tuple left_tuple, const Schema &left_schema, const Schema &right_schema) -> Tuple {
  std::vector<Value> values;
  for (uint32_t i = 0; i < left_schema.GetColumnCount(); i++) {
    values.push_back(left_tuple.GetValue(&left_schema, i));
  }
...
auto NestIndexJoinExecutor::Next(Tuple *tuple, RID *rid) -> bool { 
  while (1) {
    Tuple left_tuple;
    if (!left_child_->Next(&left_tuple, rid)) return false;
