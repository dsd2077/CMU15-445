...
TEST(LockManagerTest, AbortTest1) {
  ...
  std::thread t2([&]() {
    ...
    EXPECT_FALSE(res);  // 这里为什么期望失败？——因为下面设置了txn1的状态为ABORTED
...
TEST(LockManagerTest, MixedTest) {
  ...
  query = "delete from test_1 where x = 9;";  // seqscan算子加IS锁,delete算子加IX锁
  bustub->ExecuteSqlTxn(query, writer, txn2);

  query = "select * from test_1;";
  bustub->ExecuteSqlTxn(query, writer, txn2);  // seqscan算子检测到之前加了更高级的锁IX，放弃加IS锁

  query = "select * from test_1;";
  bustub->ExecuteSqlTxn(query, writer, txn1);  // txn1加IS锁
